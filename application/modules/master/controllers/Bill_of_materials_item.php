 ...
 */
class Bill_of_materials_item extends MY_Controller {
    ...
    function index() {
        $this->load->view('master/bom/index');
    }

//     function get_data() {
//         $this->model_bom->get_data();
//     }

    function get_bom_with_pagination() {
        $page = $this->input->post('page');
        $rows = $this->input->post('rows');
        $filterByName = $this->input->post('search');
        $result = $this->model_bom->get_bom_with_pagination($page, $rows, $filterByName);
        echo json_encode($result);
    }

    function view_list() {
        $this->load->view('master/bom/view_list');
    }

    function input_form() {
        $this->load->view('master/bom/input_form');
    }
    ...
    function add_item() {
        ...
        $isSuccess = $this->model_bom->add_item($sku, $name, $barcode, $cost, $category1, $category2, $category3, $category4, $uom_id, $carton, $inner, $retail_price, $bkp, $consignment, $type, $trading_price, $bom_status);
    ...
    function update_item() {
        ...
        $isSuccess = $this->model_bom->update_item($id, $sku, $name, $barcode, $cost, $category1, $category2, $category3, $category4, $uom_id, $carton, $inner, $retail_price, $bkp, $consignment, $type, $trading_price, $bom_status);
        $this->getMessageResult($isSuccess);
    }

    function delete_item() {
        $id = $this->input->post('id');
        $isSuccess = $this->model_bom->delete_item($id);
        $this->getMessageResult($isSuccess);
    }
